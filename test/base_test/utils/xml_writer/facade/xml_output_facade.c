                                ...
                                size_t size)
{
    ...
    const char *str = suite_name ? suite_name : "";
    while (*str)
                                     ...
                                     xml_test_result_t *test, const char *output_dir)
{
    ...
    {
        char full_name[512];
        snprintf(full_name, sizeof(full_name), "%s.%s", suite->name, test->name);
        write_json_escaped_string(f, full_name);
    }
    fprintf(f, ",\n");
    fprintf(f, "  \"name\": ");
    write_json_escaped_string(f, test->name);
    fprintf(f, ",\n");
    ...
    fprintf(f, "  \"labels\": [\n");
    fprintf(f, "    {\"name\": \"suite\",     \"value\": ");
    write_json_escaped_string(f, suite->name);
    fprintf(f, "},\n");
    fprintf(f, "    {\"name\": \"testClass\", \"value\": ");
    write_json_escaped_string(f, test->classname);
    fprintf(f, "},\n");
    fprintf(f, "    {\"name\": \"package\",   \"value\": ");
    write_json_escaped_string(f, suite->name);
    fprintf(f, "},\n");
    ...
    if (writer->environment_name[0] != '\0') {
        fprintf(f, ",\n    {\"name\": \"host\", \"value\": ");
        write_json_escaped_string(f, writer->environment_name);
        fprintf(f, "}");
    }
    ...
    if (test->result_label_count > 0) {
        ...
        for (int i = 0; i < test->result_label_count; i++) {
            fprintf(f, ",\n    {\"name\": ");
            write_json_escaped_string(f, test->result_labels[i].name);
            fprintf(f, ", \"value\": ");
            write_json_escaped_string(f, test->result_labels[i].value);
            fprintf(f, "}");
        }
    } else {
        ...
        for (int i = 0; i < suite->suite_label_count; i++) {
            fprintf(f, ",\n    {\"name\": ");
            write_json_escaped_string(f, suite->suite_labels[i].name);
            fprintf(f, ", \"value\": ");
            write_json_escaped_string(f, suite->suite_labels[i].value);
            fprintf(f, "}");
        }
    }
    fprintf(f, "\n  ],\n");
                    ...
                    if (path_len < sizeof(file_path)) {
                        ...
                        if (pipe2 && pipe2 < attach_end) {
                            ...
                            if (name_len < sizeof(attach_name)) {
                                ...
                                attach_name[sizeof(attach_name) - 1] = '\0';
                            }
                        ...
                        fprintf(f, "    {\n");
                        fprintf(f, "      \"name\": ");
                        write_json_escaped_string(f, attach_name);
                        fprintf(f, ",\n      \"source\": ");
                        write_json_escaped_string(f, rel_source);
                        fprintf(f, ",\n      \"type\": ");
                        write_json_escaped_string(f, attach_type);
                        fprintf(f, "\n    }");
                    }
                }
