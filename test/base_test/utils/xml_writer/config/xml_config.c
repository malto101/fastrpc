...
void xml_config_apply_environment_defaults(xml_config_t *config, xml_environment_t env)
{
    if (!config)
        return;
...
xml_config_t *xml_config_create_for_env(xml_environment_t env)
{
    ...
    xml_config_apply_environment_defaults(config, env);
...
void xml_config_apply_env_overrides(xml_config_t *config)
{
    ...
    /* Check for environment variable. Re-applies the full profile for the
     * requested environment, not just the label, so XML_ENV actually takes
     * effect when active_config was created for a different environment. */
    env_val = getenv("XML_ENV");
    if (env_val) {
        xml_config_apply_environment_defaults(config, xml_environment_from_string(env_val));
    }
